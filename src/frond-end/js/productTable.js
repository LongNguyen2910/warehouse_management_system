...
function getAllProducts() {
    $.ajax({
        ...
        success: function(res) {
            ...
            if (list.length === 0) {
                table = '<tr><td colspan="7" class="text-center text-muted py-4">Không có sản phẩm nào.</td></tr>';
            }

            for (var i = 0; i < list.length; ++i) {
                const p = list[i];
                table += '<tr>';
                table += '<td><i class="fab fa-react fa-lg text-info"></i> <strong>' + p.sku + '</strong></td>';
                ...
                table += '<button type="button" class="btn p-0 dropdown-toggle hide-arrow" data-bs-toggle="dropdown">';
                table += '<i class="bx bx-dots-vertical-rounded"></i></button>';
                table += '<div class="dropdown-menu">';
                table += '<a class="dropdown-item" href="javascript:void(0);" onclick="goToEdit(\'' + p.id + '\')">';
                table += '<i class="bx bx-edit-alt me-2"></i>Edit</a>';
                table += '<a class="dropdown-item text-danger" href="javascript:void(0);" onclick="confirmDelete(\'' + p.id + '\',\'' + escapeQuote(p.product_name) + '\')">';
        ...
        error: function(xhr) {
            showToast('Lỗi tải danh sách: ' + (xhr.responseJSON.message || xhr.statusText), 'danger');
        }
    });
}
...
function confirmDelete(id, productName) {
    ...
    document.getElementById('deleteModalBody').innerHTML =
        '<p class="mb-0">Bạn có chắc muốn xóa sản phẩm <strong>' + productName + '</strong>?</p>';
...
function executeDelete(id, productName) {
    ...
    $.ajax({
        ...
        success: function() {
            ...
            showToast('Xóa sản phẩm <strong>' + productName + '</strong> thành công!', 'success');
            getAllProducts();
        },
        error: function(xhr) {
...
function showToast(message, type = 'success') {
    ...
    const iconMap = { success: 'bx-check-circle', danger: 'bx-error-circle', warning: 'bx-error' };
