...
def inbound():
    ...
    success = execute_db(
    "INSERT INTO Receipts (type, warehouse_id, staff_id, partner_name) VALUES (N'INBOUND',?,?,?)",
        (warehouse, staff, partner)
    )
    if not success:
        abort(500, description="Đã xảy ra lỗi khi thêm thông tin phiếu")

    receipt = query_db("select @@IDENTITY as Id", one=True)["Id"]
