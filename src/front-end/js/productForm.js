...
function displayCategory() {
    $.ajax({
        ...
        success: function(res) {
            ...
            for (var i = 0; i < len; ++i) {
                table += '<option value="' + res[i].name + '">' + res[i].name + '</option>';
            }
...
function loadProduct(id) {
    $.ajax({
        ...
        success: function(res) {
            ...
            document.getElementById('basic-default-product-name').value = p.product_name || '';
            document.getElementById('basic-default-min-stock').value = p.min_stock || 0;
            document.getElementById('basic-default-description').value = p.description || '';
            ...
            document.getElementById('basic-default-created-at').value = formatDatetimeLocal(p.created_at);
        },
        error: function() {
...
function validate() {
    ...
    const name = document.getElementById('basic-default-product-name').value.trim();
    const category = document.getElementById('categorySelect').value;

    if (!sku) { showAlert('Vui lòng nhập SKU.', 'warning'); return false; }
    if (!name) { showAlert('Vui lòng nhập tên sản phẩm.', 'warning'); return false; }
    if (!category) { showAlert('Vui lòng chọn danh mục.', 'warning'); return false; }
    return true;
}
...
function submitForm() {
    ...
    const createdAtRaw = document.getElementById('basic-default-created-at').value;

    const data = {
        sku: document.getElementById('basic-default-sku').value.trim(),
        product_name: document.getElementById('basic-default-product-name').value.trim(),
        category_name: document.getElementById('categorySelect').value, // gửi number, không phải string
        min_stock: parseInt(document.getElementById('basic-default-min-stock').value) || 0,
        description: document.getElementById('basic-default-description').value.trim(),
    ...
    $.ajax({
        ...
        success: function() {
            ...
            window.location.href = 'product-table.html?toast=' + encodeURIComponent(msg);
        },
        error: function(xhr) {
