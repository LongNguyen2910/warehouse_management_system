...
function loadInboundReceipts() {
    $.ajax({
        ...
        error: function(xhr) {
            $('#inboundList').html('<div class="text-center text-danger py-4">Lỗi tải dữ liệu: ' + (xhr.responseJSON.message || xhr.statusText) + '</div>');
        }
    });
}
...
function loadOutboundReceipts() {
    $.ajax({
        ...
        error: function(xhr) {
            $('#outboundList').html('<div class="text-center text-danger py-4">Lỗi tải dữ liệu: ' + (xhr.responseJSON.message || xhr.statusText) + '</div>');
        }
    });
}
...
function displayReceipts(receipts, containerId, type) {
    ...
    if (!receipts || receipts.length === 0) {
        $container.html('<div class="text-center text-muted py-4">📭 Chưa có phiếu nào</div>');
        return;
    }
    ...
    for (let i = 0; i < receipts.length; i++) {
        ...
        const totalQuantity = r.items ? r.items.reduce((sum, item) => sum + (item.qty || 0), 0) : 0;
...
function deleteReceipt(id, type, event) {
    ...
    if (!confirm(`Bạn có chắc chắn muốn xóa phiếu ${type.toUpperCase()} #${id}?`)) {
        return;
    }
